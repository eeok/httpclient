...
unsigned int my_inet_addr(const char *cp) {
...
/*
  iovec[0].base = buf;
  iovec[0].len = sizeof(struct DNS_HEADER) + (len((const char*)qname) + 1) + sizeof(struct QUESTION);
  m.msgname = 0;
  m.msglen = 0;
  m.msgiov = iovec;
  m.iovlen = 1;
  ...
  m.msgcontrol = 0;
  m.msgflags = 0;
  long received_len = rcv(s, &m, 0);
    if (received_len < 0) {
        syswrite(1, "Read failed\n", 12);
        return -1;
    }
    syswrite(1, "yes???", 1);
  //move ahead othe dns header and the query field
